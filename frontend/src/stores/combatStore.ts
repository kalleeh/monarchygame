...
import { calculateTurnCost, requiresWarDeclaration, validateAttackType, type WarDeclaration } from '../../../game-data/mechanics/combat-mechanics';
    ...
    (set, get) => ({
      ...
      executeBattle: async (targetId: string) => {
        ...
        try {
          ...
          // Apply casualties to kingdom units (single source of truth)
          const kingdomUnits = useKingdomStore.getState().units;
          kingdomUnits.forEach(unit => {
            const casualties = battleResult.casualties.attacker[unit.type] || 0;
            if (casualties > 0) {
              useKingdomStore.getState().removeUnits(unit.id, casualties);
            }
          });
...
async function simulateBattle(
  ...
  // Convert defender kingdom units to Unit format
  const defenderUnits: Unit[] = [
    { id: 'def-tier1', type: 'peasant' as const, count: defenderKingdom.units.tier1, attack: 1, defense: 1, health: 10 },
    { id: 'def-tier2', type: 'militia' as const, count: defenderKingdom.units.tier2, attack: 2, defense: 3, health: 20 },
    { id: 'def-tier3', type: 'knight' as const, count: defenderKingdom.units.tier3, attack: 4, defense: 4, health: 40 },
    { id: 'def-tier4', type: 'cavalry' as const, count: defenderKingdom.units.tier4, attack: 5, defense: 2, health: 30 }
  ].filter(u => u.count > 0);
  ...
  const formationBonus = formation ? (formation.bonuses.attack / 100) : 0;
  const totalAttackerPower = attackerPower * (1 + formationBonus);
  
  // Calculate offense ratio (from combat-mechanics.ts)
  const offenseRatio = totalAttackerPower / defenderPower;
  ...
  attackerUnits.forEach(unit => {
    const casualties = Math.floor(unit.count * attackerCasualtyRate);
    if (casualties > 0) {
      attackerCasualties[unit.type] = casualties;
    }
  });
  ...
  defenderUnits.forEach(unit => {
    ...
    if (casualties > 0) {
      defenderCasualties[unit.type] = casualties;
    }
  });
  ...
  // Gold looted: 1000 per acre gained (from reference)
  const goldGained = landGained * 1000;
