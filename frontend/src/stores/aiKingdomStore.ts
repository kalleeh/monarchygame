...
function calculateNetworth(kingdom: Omit<AIKingdom, 'networth' | 'difficulty'>): number {
  ...
  const unitValue = Object.values(kingdom.units).reduce((sum, count) => sum + count * 100, 0);
...
function generateAIKingdom(
  ...
  // Use player networth directly as base (not divided by 10000)
  const baseNetworth = Math.max(100000, playerNetworth); // Minimum 100k networth
  
  // Calculate base resources from networth
  const baseLand = Math.floor(baseNetworth / 1000); // Land worth 1000 each
  const baseGold = Math.floor(baseNetworth * 0.3); // 30% of networth in gold
  const basePopulation = Math.floor(baseLand * 2); // 2 pop per land
